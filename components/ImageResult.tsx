...
import { Download, RefreshCw, ZoomIn, Loader2, Edit2 } from 'lucide-react';
...
export const ImageResult: React.FC<ImageResultProps> = ({ image, onDownload, onRetry, onUpdatePrompt }) => {
  const [promptText, setPromptText] = useState(image.prompt);

  // Sync local state if parent updates the prompt (e.g. after regeneration confirms)
  useEffect(() => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      if (promptText.trim() !== image.prompt) {
        onUpdatePrompt(image.id, promptText);
      }
    }
  };
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="relative group/prompt">
        <div className="absolute top-2 right-2 opacity-0 group-hover/prompt:opacity-100 pointer-events-none transition-opacity">
        ...
        <textarea 
          ...
          className="w-full text-xs text-stone-600 bg-transparent border border-transparent hover:border-stone-200 focus:border-stone-300 focus:bg-white rounded p-1.5 outline-none resize-none transition-all placeholder:text-stone-300 min-h-[3rem]"
          placeholder="Enter prompt..."
          disabled={image.status === 'loading'}
        />
        <p className="text-[10px] text-stone-400 text-right h-4 opacity-0 group-focus-within/prompt:opacity-100 transition-opacity px-1">
